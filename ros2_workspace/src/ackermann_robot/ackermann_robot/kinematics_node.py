...
class AckermannKinematics:
    ...
    def forward_kinematics(self, robot_state: RobotState) -> Tuple[float, float, float]:
        ...
        v_x_robot = (v_LF * np.cos(delta_LF) + 
                    v_LR * np.cos(delta_LR) + 
                    v_RF * np.cos(delta_RF) + 
                    v_RR * np.cos(delta_RR)) / 4.0
                    
        v_y_robot = (v_LF * np.sin(delta_LF) + 
                    v_LR * np.sin(delta_LR) + 
                    v_RF * np.sin(delta_RF) + 
                    v_RR * np.sin(delta_RR)) / 4.0
        
        # Angular velocity from wheel velocities and positions
        # This is more complex for 4-wheel independent drive
    ...
    def _compute_angular_velocity(self, velocities: list, steering_angles: list) -> float:
        """
        Compute angular velocity from wheel velocities and steering angles
        Using instantaneous center of rotation method
        """
        # Simplified approach: use average of left/right side contributions
        # This could be enhanced with more sophisticated algorithms
        
        # Left side contribution
        omega_left = 0.0
        if abs(steering_angles[0]) > 1e-6:  # LF has steering
            omega_left += velocities[0] * np.sin(steering_angles[0]) / (self.geo.wheelbase/2)
        if abs(steering_angles[1]) > 1e-6:  # LR has steering  
            omega_left += velocities[1] * np.sin(steering_angles[1]) / (self.geo.wheelbase/2)
            
        # Right side contribution
        omega_right = 0.0
        if abs(steering_angles[2]) > 1e-6:  # RF has steering
            omega_right += velocities[2] * np.sin(steering_angles[2]) / (self.geo.wheelbase/2)
        if abs(steering_angles[3]) > 1e-6:  # RR has steering
            omega_right += velocities[3] * np.sin(steering_angles[3]) / (self.geo.wheelbase/2)
    ...
    def inverse_kinematics(self, v_x: float, v_y: float, omega: float) -> RobotState:
        ...
        Args:
            ...
            v_y: Desired lateral velocity (m/s)  
        ...
        # For Ackermann steering, lateral velocity should be minimal
        # Focus on forward motion and rotation
        ...
        if abs(omega) < 1e-6:
            ...
            wheel_vel = v_x
            robot_state.left_front.velocity = wheel_vel
            robot_state.left_rear.velocity = wheel_vel
            robot_state.right_front.velocity = wheel_vel
            robot_state.right_rear.velocity = wheel_vel
            
        else:
            # Turning motion - compute turning radius and Ackermann angles
            if abs(v_x) > 1e-6:
                R = v_x / omega  # Turning radius
            else:
                R = self.geo.wheelbase / 2.0  # Default for pure rotation
                
            # Compute Ackermann steering angles properly
            # Both sides steer in the same direction but with different angles
            abs_R = abs(R)
            
            if omega > 0:  # Left turn (positive omega)
                # Inner wheel (left) has larger steering angle
                ...
                # Inner wheel (right) has larger steering angle
                robot_state.theta_L = -np.arctan(self.geo.wheelbase / (abs_R + self.geo.track_width/2))
                robot_state.theta_R = -np.arctan(self.geo.wheelbase / (abs_R - self.geo.track_width/2))
            ...
            # Compute wheel velocities for the turn
            self._compute_wheel_velocities_for_turn(robot_state, v_x, omega, R)
            
        return robot_state
        
    def _compute_wheel_velocities_for_turn(self, robot_state: RobotState, v_x: float, omega: float, R: float):
        """Compute individual wheel velocities for turning motion"""
        ...
        # Compute wheel velocities based on distance from turning center
        # For Ackermann steering, each wheel has different velocity based on its radius from center
        
        abs_R = abs(R)
        
        if omega > 0:  # Left turn
            # Left wheels are closer to center (smaller radius)
            R_left = abs_R - self.geo.track_width/2.0
            R_right = abs_R + self.geo.track_width/2.0
        else:  # Right turn  
            # Right wheels are closer to center (smaller radius)
            R_left = abs_R + self.geo.track_width/2.0
            R_right = abs_R - self.geo.track_width/2.0
        
        # Compute angular velocity for each side
        if abs(R_left) > 1e-6:
            omega_left = v_x / R_left
        else:
            omega_left = 0.0
            
        if abs(R_right) > 1e-6:
            omega_right = v_x / R_right
        else:
            omega_right = 0.0
        
        # Convert angular velocities to wheel velocities
        robot_state.left_front.velocity = omega_left * R_left
        robot_state.left_rear.velocity = omega_left * R_left
        robot_state.right_front.velocity = omega_right * R_right
        robot_state.right_rear.velocity = omega_right * R_right
